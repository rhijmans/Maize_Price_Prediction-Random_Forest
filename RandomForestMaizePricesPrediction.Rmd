...
### (I) The Data
There are two sets of data for this exersice: A csv(comma delimited file) with maize prices for diffrent locations in Tanzania; and several raster files of  weather elements, soil properties and distance to infrastructure/amenities that encompase Tanzania. 
...
#### 1. Raster Data Set (weather elements, soil properties and distance to amenities)
...
We use the "raster" and "rgdal" libraries for creating and manipulating spatial data in R.
```{r echo=FALSE}
library(raster)
library(rgdal)
```
